...
if (php_sapi_name() === "cli") {
    ...
    require_once '/home/erbielqv/envelop.php'; // Connessione via $pdo
    ...
    foreach ($file_list as $file) {
        ...
        if ($file !== '.' && $file !== '..' && is_file($path)) {
            $file_age = $now - filemtime($path);

            if ($file_age <= $threshold_sec) {
                $files_vivi[$file] = filemtime($path);  // Hash map!
            } else {
                if (unlink($path)) {
                    debugEcho("🗑️ Eliminato vecchio file: $file");
                }
            }
        }
    }
...
function leggiImmaginiDaDatabase(PDO $pdo) {
    ...
    if ($stmt) {
        while ($riga = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            // 🔁 Converti DATA_ORA in timestamp per confronto con filemtime
            $timestamp = strtotime($data_ora);

            $files_nel_db[$filename] = $timestamp;
        }
    }
 ...
 function sincronizzaDatabase(PDO $pdo, array $file_map_dir, array $file_map_db) {
    ...
    if (empty($file_map_db)) {
        ...
        foreach ($file_map_dir as $filename => $timestamp) {
    ...
    foreach ($file_map_db as $filename => $timestamp) {
    ...
    function aggiornaDatiMeteo(PDO $pdo) {
