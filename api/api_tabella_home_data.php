...
SELECT 
 ...
 FROM $table WHERE vento_kmh IS NOT NULL AND data_ora >= (SELECT MAX(data_ora) FROM $table
  ...
  COALESCE(
  (SELECT temperatura_C FROM $table 
   WHERE temperatura_C IS NOT NULL 
     AND data_ora >= DATE_SUB(:now_24h_plus, INTERVAL 1500 MINUTE)
     AND data_ora <  DATE_SUB(:now_24h_minus, INTERVAL 1380 MINUTE)
   ORDER BY ABS(TIME_TO_SEC(TIMEDIFF(data_ora, 
           DATE_SUB(:now_24h_center, INTERVAL 1440 MINUTE)))) ASC
   LIMIT 1),  NULL) AS temp_24h_ago,
   COALESCE(
  (SELECT pressione_hPa  FROM $table 
   WHERE pressione_hPa  IS NOT NULL 
     AND data_ora >= DATE_SUB(:now_24h_plus, INTERVAL 1500 MINUTE)
   AND data_ora <  DATE_SUB(:now_24h_minus, INTERVAL 1380 MINUTE)
 ORDER BY ABS(TIME_TO_SEC(TIMEDIFF(data_ora, 
         DATE_SUB(:now_24h_center, INTERVAL 1440 MINUTE)))) ASC
 LIMIT 1),  NULL) AS press_24h_ago,
  ...
  (
    ...
    FROM $table
    WHERE temperatura_C IS NOT NULL
      AND data_ora >= DATE_SUB(:now_1h_plus, INTERVAL 120 MINUTE)
      AND data_ora <  DATE_SUB(:now_1h_minus, INTERVAL 60 MINUTE)
    ORDER BY ABS(
      TIME_TO_SEC(
        TIMEDIFF(
          data_ora,
          DATE_SUB(:now_1h_center, INTERVAL 60 MINUTE)
        )
      )
    ) ASC
    LIMIT 1
...
";
    ...
    try {
        ...
        $stmt->execute([
    ':now_24h_ago' => $now_24h_ago,           // Mantiene la vecchia
    ':now_24h_plus' => $now_24h_plus,         // ✨ Nuova
    ':now_24h_minus' => $now_24h_minus,       // ✨ Nuova
    ':now_24h_center' => $now_24h_center,     // ✨ Nuova
    ':now_1h_ago' => $now_1h_ago,
    ':now_1h_plus' => $now_1h_plus,         // ✨ Nuova
    ':now_1h_minus' => $now_1h_minus,       // ✨ Nuova
    ':now_1h_center' => $now_1h_center,       // ✨ Nuova
    ':today' => $today,
    ':yesterday' => $yesterday
]);
    ...
    //variabile per restituire na nel caso il delta sia null
    $txtDelta24t = ($temp_delta_24h === null) ? 'N/A' : (formatDelta($temp_delta_24h) . '°C');
    $txtDelta24p = ($press_delta_24h === null) ? 'N/A' : (formatDelta($press_delta_24h) . 'hPa');
    $txtDelta1p = ($press_delta_1h === null)
    ? 'N/A'
    : formatDelta($press_delta_1h);

    $txtDelta1t = ($temp_delta_1h === null)
    ? 'N/A'
    : formatDelta($temp_delta_1h);
    ...
    $rows = [
        ...
        [
            ...
            'note' => createDeltaIndicator($temp_delta_24h) . " Δ24h(attuale-24h) = " . $txtDelta24t,
  ...
  'interactive' => [
    ...
    'clickable' => true
        ...
        [
            ...
            'note' => createPressureTrendIndicator($press_delta_24h) . " Δ24h(attuale-24h) = " . $txtDelta24p,
...
function createDeltaIndicator($deltaValue): string {
    // Se NULL o non disponibile → pallino grigio
    if ($deltaValue === null) {
        return '<svg width="12" height="12" ... title="Dato non disponibile">
                  <circle cx="6" cy="6" r="4" fill="#999999" stroke="#666" .../>
                </svg>';
    }
...
function createPressureTrendIndicator($deltaValue): string {
    ...
    // If NULL or not available, show gray circle
    if ($deltaValue === null) {
        return '<svg width="12" height="12" style="vertical-align: middle; margin-right: 5px;" title="Dato non disponibile">
                  <circle cx="6" cy="6" r="4" fill="#999999" stroke="#666" stroke-width="0.5"/>
                </svg>';
    }
    $deltaValue = floatval($deltaValue);
...
function getSolareteoricoMezzaGiornata(?PDO $pdo): array {
    ...
    try {
        ...
        $debug_radianza = [
            'tabella_usata' => $table,
            'valore_estratto' => $radianza_attuale,
            'query_start' => $now_15m_ago,
            'query_end' => $now,
            'today' => $today
        ];
...
/*DEGUG PER VEDERE SE IL DB E' TEST O NO
console.log('📊 DB table (dati_meteo):', '<?php echo table_name("dati_meteo_simignano"); ?>');
console.log('📷 DB table_name_bis (immagini):', '<?php echo table_name("DB_immagini_36h"); ?>');
*/
