...
// Version 1.18 - 18-Jan-2022 - fixes for PHP 8.1 Deprecated errata
// Version 1.19 - 27-Dec-2022 - fixes for PHP 8.2
//
...
if (isset($_REQUEST['wx']) and strtolower($_REQUEST['wx']) == 'weewx')  { $wxSoftware = 'WEEWX';  } // testing
...
if ($wxSoftware == 'WD' or $wxSoftware == 'MH' or $wxSoftware == 'WEEWX') { // Get the Weather-Display/Meteohub/WeeWX clientraw.txt data file
      ...
      $dataraw = preg_replace("/[\r\n]+[\s\t]*[\r\n]+/","\n",$dataraw);
			$dataraw = preg_replace("/\s+/",' ',$dataraw);
...
// draw a filled thermometer with scale, min max on a blank thermometer image
 ...
 $Y = (integer)((1-$Tpct)*($maxY-$minY)+$minY); // upper location for fill
...
// Draw tick marks and scale values on right
 ...
 for ($T=$Tmin;$T<=$Tmax;$T+=$Tincr) {
     ...
     $Tpct = ($T-$Tmin)/($Trange);
     $Y = (integer)((1-$Tpct)*($maxY-$minY)+$minY);
	 ...
	 if ($T == 0 or ($T % $TMajTick) == 0) { // Major Tick
        ...
        imagestring($image, $font,
            $maxX + 14,
            $Y - (integer)(ImageFontHeight($font)/2),
            sprintf( "%2d", $T),$tx);
	 } else { // Minor tick
     	imagefilledrectangle( $image,
            $maxX+7,
