...
# Version 1.12 - 14-May-2021 - added support for WeeWX plugin
# Version 1.13 - 18-Jan-2022 - fixes for PHP 8.1 errata
############################################################################
$WXSIVer = 'include-wxstatus.php - V1.13 - 18-Jan-2022';
//--self downloader --
...
// check the appropriate realtime file first
  ...
  if($SITE['WXsoftware'] == 'WEEWX' and isset($SITE['clientrawfile'])) {
	do_check($SITE['WXsoftwareLongName']." ".langtransstr("realtime"),$SITE['clientrawfile'],60+30,'file');
  }
  ...
  $WEEWXnoaaFile = 'NOAA-'.date('Y').'-'.date('m').'.txt';
  if($SITE['WXsoftware'] == 'WEEWX' and file_exists($SITE['NOAAdir'].$WEEWXnoaaFile)) {
	do_check($SITE['WXsoftwareLongName']." ".langtransstr("NOAA report"),
	  $SITE['NOAAdir'].$WEEWXnoaaFile,60*5+15,'file');
  }
